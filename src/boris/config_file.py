...
def _config_file_path() -> pl.Path:
    return pl.Path(os.environ.get("BORIS_CONFIG_PATH", str(pl.Path.home() / ".boris"))).expanduser()


def _recent_projects_file_path() -> pl.Path:
    return pl.Path(os.environ.get("BORIS_RECENT_PROJECTS_PATH", str(pl.Path.home() / ".boris_recent_projects"))).expanduser()
...
def read(self) -> None:
    ...
    ini_file_path = _config_file_path()
    ...
    recent_projects_file_path = _recent_projects_file_path()
...
def save(self, lastCheckForNewVersion=0):
    ...
    settings = QSettings(str(_recent_projects_file_path()), QSettings.Format.IniFormat)
