...
def _post_seek_frame_wait(self) -> None:
    """
    Avoid blocking the Qt event loop on embedded backend after a seek.
    """
...
def delete_selected_events(self):
    ...
    if not self.tv_events.selectedIndexes():
        ...
        view_state = self.capture_events_view_state()
        selected_rows = sorted({item.row() for item in self.tv_events.selectedIndexes()})
        ...
        self.load_tw_events(self.observationId)
        if selected_rows:
            self.restore_events_view_state(view_state, selected_row=min(selected_rows[0], max(self.tv_events.model().rowCount() - 1, 0)))
...
def edit_selected_events(self):
    ...
    if not len(tvevents_rows_to_edit):
        ...
        if dialog_window.exec_():
            ...
            self.load_tw_events(self.observationId)
            if tvevents_rows_to_edit:
                self.restore_events_view_state(view_state, selected_row=min(tvevents_rows_to_edit))
                ...
                for key in self.pj[cfg.ETHOGRAM]:
                    if self.pj[cfg.ETHOGRAM][key][cfg.BEHAVIOR_CODE] == edit_window.cobCode.currentText():
                        ...
                        if self.pj[cfg.OBSERVATIONS][self.observationId][cfg.TYPE] == cfg.MEDIA:
                            if self.playerType == cfg.MEDIA:
                                original_frame_idx = read_event_field(
                                    self.pj[cfg.OBSERVATIONS][self.observationId][cfg.EVENTS][pj_event_idx],
                                    self.pj[cfg.OBSERVATIONS][self.observationId][cfg.TYPE],
                                    cfg.FRAME_INDEX,
                                )

                                # Editing subject/comment without changing the timestamp should keep the original frame index.
                                if new_time.quantize(dec("0.001"), rounding=ROUND_DOWN) == time_value.quantize(
                                    dec("0.001"), rounding=ROUND_DOWN
                                ):
                                    event[cfg.FRAME_INDEX] = original_frame_idx
                                else:
                                    mem_time = self.getLaps()
                        ...
                        if not r:
                            flag_ok = True
                            self.restore_events_view_state(view_state, selected_row=tvevents_row)
                        break
