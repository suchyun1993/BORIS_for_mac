...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, ffmpeg_bin, parent=None):
        ...
        self.actionShortcut_list = QAction("Shortcut list", self)
        self.actionShortcut_list.setObjectName("actionShortcut_list")
        self.menuHelp.insertAction(self.actionAbout, self.actionShortcut_list)
        self.actionShortcut_list.triggered.connect(self.shortcut_list_activated)
        ...
        self.apply_obs_info_panel_style()

        # statusbat font
        status_font = QFont()
        status_font.setPointSize(self.obs_info_font_size)
    ...
    def apply_obs_info_panel_style(self) -> None:
        """
        Keep the bottom observation info panel compact and readable.
        """

        font = QFont()
        font.setPointSize(getattr(self, "obs_info_font_size", 10))
        ...
        self.lb_current_media_time.setWordWrap(False)
        self.lbFocalSubject.setWordWrap(False)
        self.lbCurrentStates.setWordWrap(False)
    ...
    def capture_events_view_state(self) -> dict | None:
        """
        Capture event table scroll position so edits do not yank the viewport.
        """

        if self.tv_events.model() is None:
            return None

        selected_rows = sorted({index.row() for index in self.tv_events.selectionModel().selectedIndexes()})
        return {
            "vertical": self.tv_events.verticalScrollBar().value(),
            "horizontal": self.tv_events.horizontalScrollBar().value(),
            "selected_rows": selected_rows,
        }

    def restore_events_view_state(self, view_state: dict | None, selected_row: int | None = None) -> None:
        """
        Restore event table scroll position after edits or deletes.
        """

        if self.tv_events.model() is None or view_state is None:
            return

        self.tv_events.verticalScrollBar().setValue(view_state.get("vertical", 0))
        self.tv_events.horizontalScrollBar().setValue(view_state.get("horizontal", 0))

        rows_to_select = []
        if selected_row is not None:
            rows_to_select = [selected_row]
        else:
            rows_to_select = view_state.get("selected_rows", [])

        if rows_to_select:
            self.tv_events.clearSelection()
            for row in rows_to_select:
                if 0 <= row < self.tv_events.model().rowCount():
                    self.tv_events.selectRow(row)
                    break

            self.tv_events.verticalScrollBar().setValue(view_state.get("vertical", 0))
            self.tv_events.horizontalScrollBar().setValue(view_state.get("horizontal", 0))

    def _event_row_is_visible(self, row: int) -> bool:
        if self.tv_events.model() is None or not (0 <= row < self.tv_events.model().rowCount()):
            return False

        top_row = self.tv_events.rowAt(0)
        bottom_row = self.tv_events.rowAt(self.tv_events.viewport().height() - 1)
        if top_row == -1 or bottom_row == -1:
            return False
        return top_row <= row <= bottom_row

    def _scroll_event_row_if_needed(self, row: int, hint=QAbstractItemView.ScrollHint.PositionAtCenter) -> None:
        if self.tv_events.model() is None or not (0 <= row < self.tv_events.model().rowCount()):
            return
        if self._event_row_is_visible(row):
            return

        index = self.tv_events.model().index(row, 0)
        self.tv_events.scrollTo(index, hint)
    ...
    def block_dockwidgets(self):
        ...
        """
        dockwidgets = [self.dwEvents, self.dwEthogram, self.dwSubjects]
        dockwidgets.extend(getattr(self, "dw_player", []))
        ...
        if self.action_block_dockwidgets.isChecked():
            observation_operations._arrange_embedded_phase1_docks(self)

    def shortcut_list_activated(self) -> None:
        dialog_ = QDialog(self)
        dialog_.setWindowTitle(f"{cfg.programName} - Shortcut list")
        dialog_.resize(860, 680)

        layout = QVBoxLayout(dialog_)
        browser = QTextBrowser(dialog_)
        browser.setReadOnly(True)
        browser.setOpenExternalLinks(True)
        browser.setHtml(self._build_shortcut_list_html())
        layout.addWidget(browser)

        dialog_.exec()

    def _build_shortcut_list_html(self) -> str:
        def display_shortcut(shortcut: str) -> str:
            if sys.platform.startswith("darwin"):
                shortcut = shortcut.replace("Ctrl", "Command")
            return html.escape(shortcut)
        ...
        for action in self.findChildren(QAction):
            ...
            shortcuts = [shortcut.toString() for shortcut in action.shortcuts() if shortcut and not shortcut.isEmpty()]
            if not shortcuts and action.shortcut() and not action.shortcut().isEmpty():
            ...
            for shortcut in shortcuts:
                ...
                if key in seen:
                    continue
                seen.add(key)
                action_rows.append((display_shortcut(shortcut), html.escape(text)))

        action_rows.sort(key=lambda item: (item[0], item[1]))

        cmd_or_ctrl = "Command" if sys.platform.startswith("darwin") else "Ctrl"
        manual_rows = [
            ("Space", "Play / Pause"),
            ...
            ("Home", "Reset playback speed to 1x"),
            ("Page Up", "Increase playback speed"),
            ("Page Down", "Decrease playback speed"),
            (f"{cmd_or_ctrl} + 0", "Reset video zoom and pan"),
            (f"{cmd_or_ctrl} + +", "Zoom in"),
            (f"{cmd_or_ctrl} + -", "Zoom out"),
            (f"{cmd_or_ctrl} + Arrow keys", "Pan video"),
            (f"{cmd_or_ctrl} + Mouse wheel", "Zoom around cursor"),
        ]

        def rows_to_html(rows):
            return "".join(
                f"<tr><td><code>{shortcut}</code></td><td>{description}</td></tr>" for shortcut, description in rows
            )

        return f"""
        <html>
          <body>
            <h2>{html.escape(cfg.programName)} shortcut list</h2>
            <p>Menu shortcuts are listed first, followed by playback and embedded-video controls.</p>
            <h3>Menu shortcuts</h3>
            <table border="1" cellspacing="0" cellpadding="6">
              <tr><th align="left">Shortcut</th><th align="left">Action</th></tr>
              {rows_to_html(action_rows)}
            </table>
            <h3>Playback and video controls</h3>
            <table border="1" cellspacing="0" cellpadding="6">
              <tr><th align="left">Shortcut</th><th align="left">Action</th></tr>
              {rows_to_html([(html.escape(shortcut), html.escape(description)) for shortcut, description in manual_rows])}
            </table>
            <h3>Project-defined keys</h3>
            <p>The single-key behavior shortcuts shown in the ethogram are project-specific, so they are not repeated here.</p>
          </body>
        </html>
        """
    ...
    def _wait_for_player_position_change(
        self,
        player: int,
        old_time: float | None,
        *,
        ...
        """
        Let the UI breathe while waiting for asynchronous seek/frame updates.

        The Qt embedded backend updates time position asynchronously, so a hard
        sleep loop on the main thread can freeze the whole window.
        """

        deadline = time.monotonic() + timeout_s
        while time.monotonic() < deadline:
            current_time = self.dw_player[player].player.time_pos
            if current_time is None:
                break
            if stop_at_zero and current_time <= 0:
                break
            if current_time != old_time:
                break
            if self.dw_player[player].player.eof_reached or self.dw_player[player].player.core_idle:
                break
            QApplication.processEvents()
            time.sleep(0.001)
        ...
        try:
            ...
            if self.dw_player[player].player.playlist_count == 1:
                if new_time < self.dw_player[player].player.duration:
                    ...
                    if player == 0:
                        self._wait_for_player_position_change(player, old_time, stop_at_zero=True)
                ...
                if new_time < self.dw_player[player].cumul_media_durations_sec[-1]:
                    for idx, d in enumerate(self.dw_player[player].cumul_media_durations_sec[:-1]):
                        if d <= new_time < self.dw_player[player].cumul_media_durations_sec[idx + 1]:
                            ...
                            if getattr(self, "use_qt_media_backend", False):
                                self._wait_for_player_position_change(player, old_time, timeout_s=0.08)
                            else:
                                time.sleep(0.5)
    ...
    def player_clicked(self, player_id: int, cmd: str) -> None:
        ...
        if getattr(self, "embedded_phase1_mode", False) and cmd in {"MBTN_LEFT_DBL", "MBTN_RIGHT_DBL"}:
            return
        ...
        if cmd == "Ctrl+WHEEL_UP":
            ...
            do_zoom_in_clicked_coords(zoom_increment=0.1)
            return
    ...
    def next_frame(self) -> None:
        ...
        if self.playerType == cfg.MEDIA:
            ...
            if not getattr(self, "use_qt_media_backend", False):
                self._wait_for_player_position_change(0, old_time)
                self.mpv_timer_out()
    ...
    def previous_frame(self) -> None:
        ...
        if self.playerType == cfg.MEDIA:
            ...
            if not getattr(self, "use_qt_media_backend", False):
                self._wait_for_player_position_change(0, old_time, stop_at_zero=True)
                self.mpv_timer_out()
    ...
    def mpv_timer_out(self, value: float | None = None, scroll_slider=True):
        ...
        if self.dw_player[0].player.time_pos is not None:  # check if video
            msg_parts = [
                f"Current media: <b>{current_media_name}</b> (#{self.dw_player[0].player.playlist_pos + 1}/{playlist_length})",
                (
                    f"Position: <b>{util.convertTime(self.timeFormat, current_media_time_pos)}</b> / "
            ...
            if self.dw_player[0].player.playlist_count > 1:
                msg_parts.append(
                    f"Total: <b>{util.convertTime(self.timeFormat, cumulative_time_pos)} / "
    ...
    def keyPressEvent(self, event) -> None:
        ...
        ctrl_or_meta = bool(modifiers & (Qt.ControlModifier | Qt.MetaModifier))
        ...
        if self.pj[cfg.OBSERVATIONS][self.observationId][cfg.TYPE] == cfg.MEDIA:
            ...
            #  jump backward
            if not ctrl_or_meta and ek == Qt.Key.Key_Down:
                self.jumpBackward_activated()
                return

            # jump forward
            if not ctrl_or_meta and ek == Qt.Key.Key_Up:
                self.jumpForward_activated()
                return

            if ctrl_or_meta:
                zoom_or_pan_handled = False
                # video zoom
                if ek in (48, Qt.Key.Key_0):  # no zoom Ctrl/Command + 0
                    ...
                    zoom_or_pan_handled = True
                zoom_step = 0.1
                if ek in (Qt.Key.Key_Plus, Qt.Key.Key_Equal):  # zoom in with + or =
                ...
                if zoom_or_pan_handled:
                    self.update_project_zoom_pan_values()
                    return
        # frame-by-frame mode
        if ek == 47 or (ek == Qt.Key.Key_Left and not ctrl_or_meta):  # / one frame back
            self.previous_frame()
            return

        if ek == 42 or (ek == Qt.Key.Key_Right and not ctrl_or_meta):  # *  read next frame
            self.next_frame()
            return
...
def main():
    ...
    # check mpv IPC mode
    requested_player_mode = os.environ.get("BORIS_PLAYER_MODE", "").strip().lower()

    qt_backend_modes = {"qt", "qtffmpeg", "embedded", "embedded-qt", "embed"}

    window.use_qt_media_backend = False
    window.MPV_IPC_MODE = False
    if requested_player_mode in qt_backend_modes:
        window.use_qt_media_backend = True
        window.MPV_IPC_MODE = False
    elif requested_player_mode in {"external", "ipc"}:
    ...
    window.allow_embed_fallback = os.environ.get("BORIS_ALLOW_EMBED_FALLBACK", "1").strip().lower() not in {"0", "false", "no"}
    window.skip_player_dock_restore = os.environ.get("BORIS_SKIP_PLAYER_DOCK_RESTORE", "0").strip().lower() in {"1", "true", "yes"}
    window.player_backend_fallback = False
    window.requested_player_mode = requested_player_mode or ("qtffmpeg" if window.use_qt_media_backend else "external")
    window.player_backend_mode = "qtffmpeg" if window.use_qt_media_backend else ("mpv-ipc" if window.MPV_IPC_MODE else "mpv-embed")
    window.embedded_phase1_mode = window.use_qt_media_backend

    if window.MPV_IPC_MODE:
        ...
        if sys.platform.startswith("darwin") and window.requested_player_mode not in qt_backend_modes:
            QMessageBox.warning(
                None,
                cfg.programName,
