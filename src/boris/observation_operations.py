...
def _player_dock_area(player_index: int) -> Qt.DockWidgetArea:
    return Qt.DockWidgetArea.TopDockWidgetArea if player_index < 4 else Qt.DockWidgetArea.BottomDockWidgetArea


def _normalize_player_docks(self) -> None:
    if not getattr(self, "skip_player_dock_restore", False):
        return

    for i, dock_widget in enumerate(self.dw_player):
        dock_widget.setFloating(False)
        self.addDockWidget(_player_dock_area(i), dock_widget)
        dock_widget.setVisible(True)


def _arrange_embedded_phase1_docks(self) -> None:
    if not getattr(self, "embedded_phase1_mode", False):
        return
    if len(getattr(self, "dw_player", [])) != 1:
        return

    player_dock = self.dw_player[0]
    ordered_docks = (self.dwEthogram, self.dwSubjects, player_dock, self.dwEvents)
    for dock_widget in ordered_docks:
        dock_widget.setFloating(False)
        dock_widget.setVisible(True)
    ...
    # Keep this as the baseline state so first open/reopen stays stable.
    self.saved_state = self.saveState()


def _ensure_ipc_player_window(self, player_index: int, media_paths: list[str]) -> None:
    if not self.MPV_IPC_MODE or not media_paths:
        return

    player = self.dw_player[player_index].player
    if not hasattr(player, "ensure_window"):
        return

    title = f"{cfg.programName} - Player #{player_index + 1} - {Path(media_paths[0]).name}"
    player.set_window_title(title)
    player.ensure_window(title=title)

    if player.playlist_count not in (None, 0):
        return

    logging.warning(f"Player #{player_index + 1} did not expose a playlist via IPC. Respawning once.")
    if not player.restart():
        logging.error(f"Player #{player_index + 1} could not be restarted.")
        return

    player.set_window_title(title)
    player.ensure_window(title=title)
    for media_path in media_paths:
        player.playlist_append(media_path)
...
def close_observation(self):
    ...
    if self.playerType == cfg.MEDIA:
        ...
        for i, player in enumerate(self.dw_player):
            if (
                ...
                if hasattr(player.player, "stop"):
                    player.player.stop()

                if self.MPV_IPC_MODE and hasattr(player.player, "terminate"):
                    try:
    ...
    if self.playerType in (cfg.MEDIA, cfg.IMAGES):
        ...
        if getattr(self, "skip_player_dock_restore", False):
            self.saved_state = self.saveState()
        ...
        if self.pj[cfg.OBSERVATIONS][obsId]["type"] == cfg.MEDIA:
            ...
            for player in self.pj[cfg.OBSERVATIONS][obsId][cfg.FILE]:
                for mediaFile in self.pj[cfg.OBSERVATIONS][obsId][cfg.FILE].get(player, []):
                    ...
                    # duration and FPS
                    try:
                        item = QTableWidgetItem(util.seconds2time(self.pj[cfg.OBSERVATIONS][obsId][cfg.MEDIA_INFO][cfg.LENGTH][mediaFile]))
                        item.setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsSelectable)
    ...
    if rv:
        ...
        if self.pj[cfg.OBSERVATIONS][new_obs_id][cfg.TYPE] == cfg.MEDIA:
            ...
            for row in range(observationWindow.twVideo1.rowCount()):
                ...
                self.pj[cfg.OBSERVATIONS][new_obs_id][cfg.MEDIA_INFO][cfg.PLAYER_PLOT_DISPLAY][
                    # observationWindow.twVideo1.cellWidget(row, cfg.PLAYER_NUMBER_IDX).currentText()
                    observationWindow.twVideo1.item(row, cfg.MEDIA_FILE_PATH_IDX).text()
...
def initialize_new_media_observation(self) -> bool:
    ...
    if getattr(self, "use_qt_media_backend", False):
        configured_players = [
            player_key
            for player_key, media_list in self.pj[cfg.OBSERVATIONS][self.observationId][cfg.FILE].items()
            if media_list
        ]
        phase1_supported = (
            configured_players == ["1"]
            and len(self.pj[cfg.OBSERVATIONS][self.observationId][cfg.FILE].get("1", [])) == 1
        )
        if not phase1_supported:
            QMessageBox.warning(
                ...
                cfg.programName,
                (
                    "The new embedded player currently supports only observations with "
                    "a single video file on Player #1.\n\n"
                    "This observation still uses a multi-player or multi-file layout."
                ),
            )
            return False
    ...
    for i in range(cfg.N_PLAYER):
        ...
        for mediaFile in self.pj[cfg.OBSERVATIONS][self.observationId][cfg.FILE][n_player]:
            ...
            if hasattr(self.dw_player[i].player, "set_media_metadata"):
                self.dw_player[i].player.set_media_metadata(media_full_path, fps=mediaFPS, duration=mediaLength / 1000)

            # add media file to playlist
            if player_media_paths:
                self.dw_player[i].player.playlist_append(media_full_path)
            else:
                if hasattr(self.dw_player[i].player, "playlist_replace"):
                    self.dw_player[i].player.playlist_replace(media_full_path)
                else:
                    self.dw_player[i].player.playlist_append(media_full_path)
            player_media_paths.append(media_full_path)
        ...
        _ensure_ipc_player_window(self, i, player_media_paths)
        ...
        if self.MPV_IPC_MODE and hasattr(self.dw_player[i].player, "prime_frame"):
            self.dw_player[i].player.prime_frame(
                float(self.pj[cfg.OBSERVATIONS][self.observationId].get(cfg.OBSERVATION_TIME_INTERVAL, [0, 0])[0])
            )
    ...
    _normalize_player_docks(self)
    self.block_dockwidgets()
    _arrange_embedded_phase1_docks(self)

    for player in self.dw_player:
        player.setVisible(True)
        if self.MPV_IPC_MODE and hasattr(player.player, "ensure_window"):
            player.player.ensure_window(title=player.windowTitle())
        ...
        if "error" not in r:
            ...
            self.pj[cfg.OBSERVATIONS][media_file] = {
                ...
                cfg.MEDIA_INFO: {
                    ...
                    cfg.PLAYER_PLOT_DISPLAY: {media_file: ",".join(player_display)},
                },
            }
