...
from . import ipc_mpv, qt_ffmpeg_player_backend
...
class DW_player(QDockWidget):
    ...
    def _create_player_backend(self, parent):
        if getattr(parent, "use_qt_media_backend", False):
            return qt_ffmpeg_player_backend.QtFFmpegPlayerBackend(self.videoframe, parent=parent, player_id=self.id_)

        if parent.MPV_IPC_MODE:
            return ipc_mpv.IPC_MPV(socket_path=f"{cfg.MPV_SOCKET}{self.id_}", parent=parent, player_id=self.id_)
        ...
        try:
            ...
            if not getattr(parent, "allow_embed_fallback", True):
                raise

            logging.exception(f"Embedded mpv init failed for player #{self.id_ + 1}; falling back to IPC mode")
            parent.MPV_IPC_MODE = True
            parent.player_backend_fallback = True
            return ipc_mpv.IPC_MPV(socket_path=f"{cfg.MPV_SOCKET}{self.id_}", parent=parent, player_id=self.id_)
    ...
    def __init__(self, id_, parent=None):
        ...
        self.videoframe.setObjectName(f"videoframe{self.id_ + 1}")

        self.player = self._create_player_backend(parent)
